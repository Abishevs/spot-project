...
use crate::{
    cli::{Config, ProjectCommands},
    fetcher::DaemonClient,
    picker::show_picker_project,
};
use std::io;
    ...
    pub fn handle_pomodoro(&mut self, command: &PomodoroCommands) -> io::Result<String> {
        let res = match command {
            PomodoroCommands::Start {
                ...
                println!(
                    "Starting Pomodoro for {} minutes, break after {} minutes",
                    duration, break_time
                );
        ...
        let res = match cmd {
            SessionCommands::Start => {
                ...
                match selected_project_adapter {
                    Some(project) => {
                        let command = commands::SessionCommand::Start {
